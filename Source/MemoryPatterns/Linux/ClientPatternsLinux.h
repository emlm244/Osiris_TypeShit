...
struct ClientPatterns {
    [[nodiscard]] static consteval auto addClientPatterns(auto clientPatterns) noexcept
    {
        return clientPatterns
            ...
            .template addPattern<WorldToProjectionMatrixPointer, CodePattern{"00 4C 8D 05 ? ? ? ? 48 8D 0D 9B"}.add(4).abs()>()
            .template addPattern<ViewToProjectionMatrixPointer, CodePattern{"0D ? ? ? ? 48 8B 39 48 85 FF 74 BE"}.add(1).abs()>()
